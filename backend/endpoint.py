...
agent_context = '''give short summary of the text content that the user gives you.
do not respond in JSON format, return only text.
you must also help user by answering queries only related to the given data.
Dont use double-asterik i.e '**' in the response texts.'''
...
chat_history.append(SystemMessage(agent_context))

def clear():
    chat_history = [SystemMessage(agent_context),]
if __name__ == "__main__":
    message = "";
    response = gemini_llm.invoke(message).content
    # for i in response:
    #     print(i,end="\n")
    # print(type(response))
